...
| Waybill printable | [x] | `src/components/PrintableWaybill.tsx` mirrors `sample_waybill.pdf` — DATE/WAYBILL NO header, FROM (sending location + address) / TO (receiving location + address) side-by-side on the upper page, S/N-PART NO-PART DESCRIPTION-QTY-REMARKS table (the UNIT column was removed on request on both the print and the on-screen Transfer Items table; the dispatch form still records a unit), dispatched/received signature block. Addresses snapshotted as `fromAddress/toAddress` on `warehouseTransfers` (backfilled from `warehouses` in `get` for legacy waybills). |
| Waybill locations: dropdown + inline add | [x] | From/To are dropdowns over `warehouses` (name + address shown, address preview under each). Any transfer writer (CSR, Inventory Manager, Admin) can add a new location via top-level `Add location` or inline `+ New location` inside the transfer form; `warehouses:create` widened from admin-only to CSR/Inventory/Admin. Address required so every waybill prints one. Detail page Summary shows both addresses. |
| Invoice customer + parts columns (review feedback Sep 2026) | [x] | `PrintableInvoice.tsx`: Billed To now shows name + address + phone (+ email) and explicit `Invoice No.`; line table is S/N-Part No-Item Description-Qty-Unit Price-Total (Part No parsed from `CODE - description` snapshot, `—` for labour). On-screen final-invoice table in `job.$id.tsx` matches. |
