...
function SecurityPage() {
  ...
  const setup = useMutation({
    mutationFn: useConvexMutation((api as any).twoFactor.setup) as any,
  });
  const verifySetup = useMutation({
    mutationFn: useConvexMutation((api as any).twoFactor.verifySetup) as any,
  });
  const disable = useMutation({
    mutationFn: useConvexMutation((api as any).twoFactor.disable) as any,
  });
  const regen = useMutation({
    mutationFn: useConvexMutation(
      (api as any).twoFactor.regenerateBackupCodes,
    ) as any,
  });
  ...
  async function startSetup() {
    try {
      ...
      toast.error(e?.data ?? e?.message ?? "Failed");
    }
  }
  ...
  async function confirmSetup(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(e?.data ?? e?.message ?? "Invalid code");
    }
  }
          ...
          {pending && (
            <div className="rounded-xl border border-line p-4 space-y-3">
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="text-xs text-mute break-all max-w-full">
                  Secret:{" "}
                  <span className="font-mono text-ink">{pending.secret}</span>
                </div>
                <div className="text-xs text-mute break-all">
                  URI: <span className="font-mono">{pending.uri}</span>
                </div>
              </div>
            ...
            <div className="space-y-3">
              ...
              {regenCodes && (
                <div className="grid grid-cols-2 gap-2 font-mono text-sm">
                  {regenCodes.map((c) => (
                    <span key={c} className="border rounded px-2 py-1 bg-white">
                      {c}
                    </span>
                  ))}
                </div>
...
function LocalQr({ uri }: { uri: string }) {
  ...
  useEffect(() => {
    if (ref.current)
      void QRCode.toCanvas(ref.current, uri, { width: 180, margin: 1 });
  }, [uri]);
